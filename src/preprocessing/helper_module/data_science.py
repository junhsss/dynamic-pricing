...
[20200129] : to_hdfs, from_hdfs에 pickle format 추가
...
class call_data():
    ...
    def from_hdfs(self, hdfs_path, lower=True,daily_data=False, parquet_flag=True):
        '''
        if daily_data = True --> parquet 파일을 한번 더 찾으러 내려감
        (19-09-01 밑 parquet file) 
        
        pickle로 저장할 경우 : parquet_flag=False
        '''
        hdfs = HDFileSystem(host=self.host, port=self.port, driver=self.driver,pars=self.pars)
    ...
    def to_hdfs(self, df,save_path,file_format='parquet'):
        '''
        file format can be ... : 'parquet', 'csv', 'pickle(sav)'
        ...
        with hdfs.open(save_path,mode='wb') as f:
            if file_format=='parquet':
                ...
                pickle.dump(df,f)
                print('successfully saved as pickle(sav) format!')
